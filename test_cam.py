...
prototxtPath = os.path.join(
    os.getcwd(), 'deploy.prototxt')
weightsPath = os.path.join(
    os.getcwd(), 'res10_300x300_ssd_iter_140000_fp16.caffemodel')
...
first_vid_source = cv2.VideoCapture('http://192.168.1.100:81/stream')
second_vid_source = cv2.VideoCapture('http://192.168.1.200:81/stream')
...
time.sleep(1)
requests.get('http://192.168.1.100/control?var=verticalservo&val=45')
...
def detect_faces():
    ...
    # lock variables
    global first_vid_source, second_vid_source, outputFrame, outputFrame_second, lock

    # loop over frames from the video stream
    while cv2.waitKey(5):
        has_frame, frame = first_vid_source.read()
        if not has_frame:
            break
        ...
        with lock:
            try:
                outputFrame = face_recognition(
                    frame, frame_height, frame_width, '192.168.1.100')
            except:
                first_vid_source.release()
                first_vid_source = cv2.VideoCapture(
...
def detect_faces2():
    ...
    # loop over frames from the video stream
    while cv2.waitKey(1):
        has_frame_second, frame_second = second_vid_source.read()
        if not has_frame_second:
            break

        frame_second = cv2.flip(frame_second, 0)
        frame_height_second = frame_second.shape[0]
        frame_width_second = frame_second.shape[1]
        with lock:
            try:
                outputFrame_second = face_recognition(
                    frame_second, frame_height_second, frame_width_second, '192.168.1.200')
            except:
                second_vid_source.release()
                second_vid_source = cv2.VideoCapture(
                    'http://192.168.1.200/81/stream')
